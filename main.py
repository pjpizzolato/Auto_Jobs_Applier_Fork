    ...
    def validate_config(config_yaml_path: Path) -> dict:
        ...
        for key, expected_type in required_keys.items():
            if key not in parameters:
                ...
                if key == "Salary" and isinstance(parameters[key], (int, float)):
        ...
        if parameters["Salary"] not in approved_salary:
            raise ConfigError(
                f"Invalid salary value in config file {config_yaml_path}. "
                f"Current value: {parameters['Salary']}. "
                f"Must be one of: {approved_salary}"
            )
...
def main(resume: Path = None):
    try:
        ...
        logger.error(
            f"Refer to the configuration guide for troubleshooting: https://github.com/feder-cr/AIHawk_AIHawk_automatic_job_application/blob/main/readme.md#configuration {str(ce)}"
        )
    except FileNotFoundError as fnf:
        logger.error(f"File not found: {str(fnf)}")
        logger.error("Ensure all required files are present in the data folder.")
        logger.error(
            "Refer to the file setup guide: https://github.com/feder-cr/AIHawk_AIHawk_automatic_job_application/blob/main/readme.md#configuration"
        )
    except RuntimeError as re:
        logger.error(f"Runtime error: {str(re)}")

        logger.error(
            "Refer to the configuration and troubleshooting guide: https://github.com/feder-cr/AIHawk_AIHawk_automatic_job_application/blob/main/readme.md#configuration"
        )
    except Exception as e:
        logger.error(f"An unexpected error occurred: {str(e)}")
        logger.error(
            "Refer to the general troubleshooting guide: https://github.com/feder-cr/AIHawk_AIHawk_automatic_job_application/blob/main/readme.md#configuration"
        )
